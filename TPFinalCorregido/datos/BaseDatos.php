...
class BaseDatos {
    ...
    public  function Iniciar(){
        ...
        if ($conexion){
            ...
            $this->ERROR =  mysqli_connect_errno() . ": " . mysqli_connect_errno(); /*mysqli_errno($conexion) . ": " .mysqli_error($conexion)*/;
        }
        return $resp;
    }
    ...
    public function devuelveIDInsercion($consulta){
        ...
        $this->QUERY = $consulta;
        if ($this->RESULT = mysqli_query($this->CONEXION,$consulta)){
            $id = mysqli_insert_id($this->CONEXION); // Obtiene el ID del último registro insertado
            $resp =  $id;
        } else {
            $this->ERROR =mysqli_errno( $this->CONEXION) . ": " . mysqli_error( $this->CONEXION);
